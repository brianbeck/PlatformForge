...
PlatformForge/
├── README.md
├── configure.sh                 # Pre-bootstrap: checks deps, sets repo URL
├── .gitignore
...
│       └── argocd_bootstrap/     # Installs Argo CD, patches + applies manifests
│
├── argocd/                      # Argo CD configuration (GitOps-managed)
│   ├── projects/
│   │   ├── platform-stage.yml   # AppProject for stage (scoped RBAC)
│   │   └── platform-prod.yml   # AppProject for prod (scoped RBAC + sync windows)
│   └── apps/
│       ├── stage/
│       │   ├── falco.yml             # Application: Falco stage
│       │   ├── gatekeeper.yml      # Application: Gatekeeper stage
│       │   └── prometheus-stack.yml # Application: Prometheus stage
│       └── prod/
│           ├── falco.yml             # Application: Falco prod
│           ├── gatekeeper.yml      # Application: Gatekeeper prod
│           └── prometheus-stack.yml # Application: Prometheus prod
│
...
The configure script checks dependencies, asks for your repo URL, validates it, and updates all manifests.
...
3. Patch Application manifests for your model (namespaces, servers)
4. Install Argo CD via Helm with Notifications enabled
5. Dry-run validate all manifests before applying
6. Apply AppProjects and Applications
...
Open https://localhost:8080 and verify that Falco and Gatekeeper Applications appear and are syncing.
